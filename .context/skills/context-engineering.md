...
Every time you implement a story you MUST emit a structured decision log
alongside the code. The log explains WHY each file changed, which patterns
you followed, which alternatives you rejected, and how every acceptance
criterion is covered. The Code Review page renders the log next to the
diff so the reviewer can read the agent's reasoning instead of having to
re-derive it from the prompt.
...
```
.context/manufacture/{story_id}/decision.json
```

The path is relative to the project repository root. The `{story_id}` is
the story you implemented (the same id used in your branch name). Create
the parent directory if it does not exist.

### Schema (v1.0)

The file MUST validate against `apps.athena.manufacture.decision_log.DecisionLog`.
Required fields:

| Field | Type | Notes |
|---|---|---|
| `schema_version` | string literal `"1.0"` | Bump only when the schema itself changes. |
| `story_id` | string | The story id you implemented. Matches the path. |
| `story_title` | string | Human-readable title; rendered as the card heading. |
| `approach` | string | 1-3 sentences. What you built and the high-level shape. |
| `patterns_followed` | list[PatternRef] | Patterns you applied. Each `{name, source_skill, applied_to_file?}`. |
| `alternatives_rejected` | list[RejectedAlternative] | Approaches considered but not taken. Each `{name, reason, would_have_required}`. |
| `files_created` | list[FileTouch] | New files. Each `{path, purpose}`. |
| `files_modified` | list[FileTouch] | Existing files you changed. Same shape. |
| `tests_written` | list[TestRef] | Test files. Each `{path, covers_acs: [ac_id...]}`. |
| `acceptance_criteria_coverage` | list[ACCoverage] | EXACTLY one entry per AC on the story. Each `{ac_id, ac_text, covered_by_files, confidence: high|medium|low, notes}`. |
| `open_questions` | list[OpenQuestion] | Ambiguity you proceeded past. Each `{question, why_proceeded, suggested_owner?}`. |
| `confidence` | enum `high`/`medium`/`low` | Your own self-assessment of the whole implementation. |
| `reasoning_notes` | string | Free-form notes for the reviewer. Trade-offs, edge cases, anything that wouldn't be obvious from the diff. |
| `generated_at` | string | ISO-8601 timestamp at write time. Use `datetime.now(UTC).isoformat()`. |
| `generated_by` | string | The agent identifier (e.g. `cline-cerebras-glm-4.7`). |

### Coverage rule

`acceptance_criteria_coverage` MUST contain one entry for every AC the
story carries. The pre-merge quality gate enforces this by count. A
shorter list (you covered 2 of 3 ACs) trips the gate and the story is
bounced back. Use `confidence: low` rather than omitting an entry when
you are unsure your implementation satisfies the AC — that flags the AC
for closer review without blocking the merge.

### When to write it

Write the file BEFORE running your post-task self-check. The gate runs
after self-check; if the file is missing or malformed the gate fails and
the integration branch is not opened.
...
{
  "schema_version": "1.0",
  "story_id": "story-greeting-1",
  "story_title": "Implement greeting endpoint",
  "approach": "Added a FastAPI route /greet that echoes the supplied name. Validation lives in a Pydantic request model so the handler stays thin.",
  "patterns_followed": [
    {"name": "thin-handler", "source_skill": "code-conventions.md", "applied_to_file": "src/api/greet.py"}
  ],
  "alternatives_rejected": [
    {"name": "raw-flask-blueprint", "reason": "project standardises on FastAPI per architecture-strategy.md", "would_have_required": "swapping the app factory + introducing a new dependency"}
  ],
  "files_created": [
    {"path": "src/api/greet.py", "purpose": "Route handler for POST /greet."},
    {"path": "tests/test_greet.py", "purpose": "Unit tests covering all three ACs."}
  ],
  "files_modified": [
    {"path": "src/app.py", "purpose": "Register the new router."}
  ],
  "tests_written": [
    {"path": "tests/test_greet.py", "covers_acs": ["ac-0", "ac-1", "ac-2"]}
  ],
  "acceptance_criteria_coverage": [
    {"ac_id": "ac-0", "ac_text": "Given a user, when they greet, then echo back", "covered_by_files": ["src/api/greet.py", "tests/test_greet.py"], "confidence": "high", "notes": ""},
    {"ac_id": "ac-1", "ac_text": "Given an empty name, when they greet, then prompt", "covered_by_files": ["src/api/greet.py", "tests/test_greet.py"], "confidence": "high", "notes": ""},
    {"ac_id": "ac-2", "ac_text": "Given an unknown locale, when they greet, then default", "covered_by_files": ["src/api/greet.py", "tests/test_greet.py"], "confidence": "medium", "notes": "Locale fallback list is hard-coded; revisit when i18n config lands."}
  ],
  "open_questions": [
    {"question": "Should the greeting be localised based on the Accept-Language header?", "why_proceeded": "Story did not mention i18n; defaulted to English.", "suggested_owner": "PM"}
  ],
  "confidence": "high",
  "reasoning_notes": "Chose to log the prompt as a 422 rather than a 400 because the empty name is a validation failure, not a malformed request.",
  "generated_at": "2026-05-14T12:34:56+00:00",
  "generated_by": "cline-cerebras-glm-4.7"
}
```

If any required field is missing, malformed, or the AC coverage is
incomplete, the quality gate blocks the merge and surfaces the precise
...
1. Run `python3 -m compileall <every modified Python file>` — must exit 0.
2. Run the project's test discovery command and report the number of tests
   collected. ZERO tests collected is NEVER success.
3. Run at least one smoke test that exercises the main feature path
   without mocking the method under test.
4. Inspect your diff for `__pycache__/`, `*.pyc`, `*.pyo`. Remove them.
5. Search your committed code for hard-coded ISO timestamps
   (`YYYY-MM-DDTHH:MM:SSZ`). Replace with `datetime.now(UTC).isoformat()` or
   inject a clock dependency.
6. If you defined an interface like `module.execute(context)`, declare it
   as a `typing.Protocol`. Don't leave the contract implicit.
7. If you implemented a context manager interface (`with ...:`), implement
   `__enter__` and `__exit__` OR use `@contextlib.contextmanager`. Never
   return a bare object and expect `with` to work.
8. If you add a logging handler, remove it in a `finally` block.

If any check fails, fix it BEFORE returning. Do not present incomplete
work as done.
