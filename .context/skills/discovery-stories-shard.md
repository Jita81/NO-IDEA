...
**Size the number of stories to the feature's complexity — do NOT cap at 1.**
Use the feature's `acceptance_criteria`, `estimated_size`, and
`description` as your guide:

- Small / trivial feature (1-2 AC, XS-S size): **1-2 stories**.
- Medium feature (3-4 AC, M size): **3-6 stories**.
- Complex feature (5+ AC, L-XL size, or multiple user flows): **7-12 stories**.

A trivial feature genuinely yields one story; a complex feature MUST
yield seven, ten, or twelve. Err toward more thin slices — the whole
point of sharding is to give you context budget to fully decompose this
one feature. Under-decomposing in the shard means the merge step can't
recover the missing slices.
