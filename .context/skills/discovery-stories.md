...
**Size the number of stories to each feature's complexity — do NOT cap at 1.** Read the feature's acceptance criteria, estimated_size, and description, then pick the count:

- Small / trivial feature (1-2 AC, XS-S size): **1-2 stories**.
- Medium feature (3-4 AC, M size): **3-6 stories**.
- Complex feature (5+ AC, L-XL size, or spans multiple user flows / integration points): **7-12 stories**.

Err toward MORE thin slices when in doubt — under-decomposition hides delivery risk. A feature whose acceptance criteria genuinely needs 10 stories to ship MUST get 10 stories; never collapse it to 1 because the output feels long.
