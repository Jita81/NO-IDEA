...
## Phase awareness — depth rules (lower bounds, not caps)

The caller passes `{{phase}}`. It is one of `prototype`, `mvp`, `mmp`,
`product`. Use the phase to pick the FLOOR of the story count per
feature — but the ceiling is set by the feature's actual complexity,
**not** by the phase. **Never cap at 1 just because the phase is early.**

- **prototype** — **at least 1 story per feature** (demo-level
  happy path). Complex prototype features still get multiple stories.
- **mvp** — **at least 2 stories per feature**. Happy path plus the
  most load-bearing validation and failure case.
- **mmp** — **at least 3 stories per feature**. Must include error
  and retry paths alongside the happy path.
- **product** — **at least 4 stories per feature**. Must include
  scale/ops slices (telemetry, capacity, failover) alongside the
  functional work.

Within those floors, size the count to the feature's complexity:

- Small / trivial feature (1-2 AC, XS-S `estimated_size`): the floor
  (e.g. 1 for prototype, 2 for mvp).
- Medium feature (3-4 AC, M): 3-6 stories.
- Complex feature (5+ AC, L-XL, or multiple user flows / integration
  points): 7-12 stories.

A complex MVP feature gets 7-12 stories — not 2 just because the phase
table starts at 2. Under-decomposition is the dominant failure mode in
discovery; err toward more thin slices.
